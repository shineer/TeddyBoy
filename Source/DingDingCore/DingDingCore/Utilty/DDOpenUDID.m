...
static NSString * kDDOpenUDIDSessionCache = nil;
static NSString * const kDDOpenUDIDDescription = @"OpenUDID_with_iOS6_Support";
static NSString * const kDDOpenUDIDKey = @"OpenUDID";
static NSString * const kDDOpenUDIDSlotKey = @"OpenUDID_slot";
static NSString * const kDDOpenUDIDAppUIDKey = @"OpenUDID_appUID";
static NSString * const kDDOpenUDIDTSKey = @"OpenUDID_createdTS";
static NSString * const kDDOpenUDIDOOTSKey = @"OpenUDID_optOutTS";
static NSString * const kDDOpenUDIDDomain = @"org.OpenUDID";
static NSString * const kDDOpenUDIDSlotPBPrefix = @"org.OpenUDID.slot.";
static int const kDDOpenUDIDRedundancySlots = 100;

@interface DDOpenUDID (Private)
...
+ (void) _setDict:(id)dict forPasteboard:(id)pboard {
#if TARGET_OS_IPHONE || TARGET_IPHONE_SIMULATOR		
    [pboard setData:[NSKeyedArchiver archivedDataWithRootObject:dict] forPasteboardType:kDDOpenUDIDDomain];
#else
...
+ (NSMutableDictionary*) _getDictFromPasteboard:(id)pboard {
#if TARGET_OS_IPHONE || TARGET_IPHONE_SIMULATOR	
    id item = [pboard dataForPasteboardType:kDDOpenUDIDDomain];
#else
    ...
    if (item) {
        @try{
            ...
            DDOpenUDIDLog(@"Unable to unarchive item %@ on pasteboard!", [pboard name]);
            item = nil;
        }
    }
...
//
+ (NSString*) value {
    return [DDOpenUDID valueWithError:nil];
}

+ (NSString*) valueWithError:(NSError **)error {

    if (kDDOpenUDIDSessionCache!=nil) {
        if (error!=nil)
            *error = [NSError errorWithDomain:kDDOpenUDIDDomain
                                         code:kDDOpenUDIDErrorNone
                                     userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"OpenUDID in cache from first call",@"description", nil]];
        return kDDOpenUDIDSessionCache;
    }
    ...
    //
    NSString * appUID = [defaults objectForKey:kDDOpenUDIDAppUIDKey];
    if(appUID == nil)
    {
    ...
    //
    id localDict = [defaults objectForKey:kDDOpenUDIDKey];
    if ([localDict isKindOfClass:[NSDictionary class]]) {
        ...
        openUDID = [localDict objectForKey:kDDOpenUDIDKey];
        myRedundancySlotPBid = [localDict objectForKey:kDDOpenUDIDSlotKey];
        optedOutDate = [localDict objectForKey:kDDOpenUDIDOOTSKey];
        optedOut = optedOutDate!=nil;
        DDOpenUDIDLog(@"localDict = %@",localDict);
    }
    ...
    NSMutableDictionary* frequencyDict = [NSMutableDictionary dictionaryWithCapacity:kDDOpenUDIDRedundancySlots];
    for (int n=0; n<kDDOpenUDIDRedundancySlots; n++) {
        NSString* slotPBid = [NSString stringWithFormat:@"%@%d",kDDOpenUDIDSlotPBPrefix,n];
#if TARGET_OS_IPHONE || TARGET_IPHONE_SIMULATOR
        UIPasteboard* slotPB = [UIPasteboard pasteboardWithName:slotPBid create:NO];
#else
        NSPasteboard* slotPB = [NSPasteboard pasteboardWithName:slotPBid];
#endif
        DDOpenUDIDLog(@"SlotPB name = %@",slotPBid);
        if (slotPB==nil) {
            // assign availableSlotPBid to be the first one available
            if (availableSlotPBid==nil) availableSlotPBid = slotPBid;
        } else {
            NSDictionary* dict = [DDOpenUDID _getDictFromPasteboard:slotPB];
            NSString* oudid = [dict objectForKey:kDDOpenUDIDKey];
            DDOpenUDIDLog(@"SlotPB dict = %@",dict);
            if (oudid==nil) {
            ...
            NSString* gid = [dict objectForKey:kDDOpenUDIDAppUIDKey];
            if (gid!=nil && [gid isEqualToString:appUID]) {
                ...
                if (optedOut) {
                    optedOutDate = [dict objectForKey:kDDOpenUDIDOOTSKey];
                    optedOut = optedOutDate!=nil;   
                }
            }
        }
    ...
    if (openUDID==nil) {        
        if (mostReliableOpenUDID==nil) {
            ...
            //
            openUDID = [DDOpenUDID _generateFreshOpenUDID];
        } else {
        ...
        if (localDict==nil) { 
            ...
            [localDict setObject:openUDID forKey:kDDOpenUDIDKey];
            [localDict setObject:appUID forKey:kDDOpenUDIDAppUIDKey];
            [localDict setObject:[NSDate date] forKey:kDDOpenUDIDTSKey];
            if (optedOut) [localDict setObject:optedOutDate forKey:kDDOpenUDIDTSKey];
    ...
    //
    DDOpenUDIDLog(@"Available Slot %@ Existing Slot %@",availableSlotPBid,myRedundancySlotPBid);
    if (availableSlotPBid!=nil && (myRedundancySlotPBid==nil || [availableSlotPBid isEqualToString:myRedundancySlotPBid])) {
        ...
        //
        if (localDict) {
            [localDict setObject:availableSlotPBid forKey:kDDOpenUDIDSlotKey];
            saveLocalDictToDefaults = YES;
        }
        ...
        if (openUDID && localDict)
            [DDOpenUDID _setDict:localDict forPasteboard:slotPB];
    }

    // Save the dictionary locally if applicable
    //
    if (localDict && saveLocalDictToDefaults)
        [defaults setObject:localDict forKey:kDDOpenUDIDKey];
    ...
    //
    if (optedOut) {
        if (error!=nil) *error = [NSError errorWithDomain:kDDOpenUDIDDomain
                                                     code:kDDOpenUDIDErrorOptedOut
        ...
        kDDOpenUDIDSessionCache = [[NSString stringWithFormat:@"%040x",0] retain];
        return kDDOpenUDIDSessionCache;
    }
    ...
    if (error!=nil) {
        if (isCompromised)
            *error = [NSError errorWithDomain:kDDOpenUDIDDomain
                                         code:kDDOpenUDIDErrorCompromised
    ...
    kDDOpenUDIDSessionCache = [openUDID retain];
    return kDDOpenUDIDSessionCache;
}
...
+ (void) setOptOut:(BOOL)optOutValue {
    ...
    id dict = [defaults objectForKey:kDDOpenUDIDKey];
    ...
    if (optOutValue)
        [dict setObject:[NSDate date] forKey:kDDOpenUDIDOOTSKey];
    else
        [dict removeObjectForKey:kDDOpenUDIDOOTSKey];

  	// store the dictionary locally
    [defaults setObject:dict forKey:kDDOpenUDIDKey];
    
    DDOpenUDIDLog(@"Local dict after opt-out = %@",dict);
    
    // reset memory cache 
    kDDOpenUDIDSessionCache = nil;
